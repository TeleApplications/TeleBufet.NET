...
{
    public sealed class SelectQuery<T> :  QueryBuilder<T> where T : ITable 
    {
        ...
        public override string CreateQuery()
        {
			...
			for (int i = 0; i < columns.Length; i++)
			{
                ...
                if (keyColumns[i] is not null) 
                {
                    ...
                    joinString = joinString + $" INNER JOIN {(keyColumns[i].ConnectionTableType.GetCustomAttribute(typeof(TableAttribute)) as TableAttribute).TableName} {columnKeyJoinName} ON {columnKeyJoinName}.{columns[i].Name} = {GetTableName()}.{columns[i].Name}";
                }
                selectQuery = selectQuery + $"{columnName}, ";
			}
            selectQuery = $"{selectQuery[0..(selectQuery.Length - 2)]}  FROM {GetTableName()} {joinString} ORDER BY {columns[0].Name}";
            return selectQuery;
        }
    }
}
